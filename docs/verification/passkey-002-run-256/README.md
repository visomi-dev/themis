# PASSKEY-002 RUN-256 evidence

This report addresses REVW-197 only. Product scope is unchanged; no Angular,
...
```text
pnpm exec node --experimental-strip-types scripts/operational-workspace-security-scan.ts docs/verification/passkey-002-run-256/openapi/openapi-report/raw
```

`status: PASS`, `filesScanned: 2`, `findings: []`.

```text
pnpm exec node --experimental-strip-types scripts/operational-workspace-security-scan.ts docs/verification/passkey-002-run-256/fuzz/openapi-report/raw
```
...
| Phase | Work item ID(s)          | Before REVW-197                      | Before coverage/gaps                                                       | After RUN-256                             | After coverage/gaps                                                                    |
| ----- | ------------------------ | ------------------------------------ | -------------------------------------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------- |
...
| api         | `PZS005_RUN_ID=RUN-256 PZS005_ARTIFACT_DIR=docs/verification/passkey-002-run-256/openapi GATEWAY_PORT=8084 pnpm exec nx run api-e2e:openapi --skip-nx-cache`                                        | PASS: 12/12 operations, 1065/1065 cases; retained report above |
...
| security    | Both exact scan commands above, plus `PZS005_RUN_ID=RUN-256 PZS005_ARTIFACT_DIR=docs/verification/passkey-002-run-256/fuzz GATEWAY_PORT=8085 pnpm exec nx run api-e2e:openapi-fuzz --skip-nx-cache` | PASS: 84/84 fuzz cases; both scans PASS with zero findings     |
