...
import type { AuthChallenge, AuthMode, AuthUser, SessionResponse } from './auth.models';
...
export class AuthStateService {
  ...
  private readStoredChallenge() {
    ...
    const stored = window.sessionStorage.getItem('themis.pendingChallenge');
    ...
    try {
      return JSON.parse(stored) as AuthChallenge;
    } catch {
      window.sessionStorage.removeItem('themis.pendingChallenge');
      return null;
    }
  }
  ...
  private setPendingChallenge(challenge: AuthChallenge | null) {
    ...
    window.sessionStorage.setItem('themis.pendingChallenge', JSON.stringify(challenge));
  }
}
